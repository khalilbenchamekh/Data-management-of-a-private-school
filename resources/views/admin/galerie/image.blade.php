...
@section('content')
			...
			<h3 class="title is-3">Modifier une image<a class="button is-primary pull-right" href="{{ url('articles/' . $post->ID) }}" target="_blank">visualiser image</a></h3>

			{{ Form::open(['method' => 'put', 'url' => route('admin.galerie.update', $post), 'files' => true]) }}



				<div class="columns">
					<div class="column is-9">
						{{ Form::label('title', 'titre de l image : ', ['class' => 'label']) }}
						<p class="control">
							{{ Form::text('title', $post->post_title, ['class' => 'input']) }}
						</p>

						{{ Form::label('content', 'contenu de l image : ', ['class' => 'label']) }}
						<p class="control">
